...
| 16 | drills.html: next concept set = Key Signatures (infographic + drill) on the Phase 94 engine | Next build -- agreed 2026-09-23 | Reuse `KeySigStaff`, `SHARP_ORDER`/`FLAT_ORDER` (already PDF-checked in Phase 90) as the shared data so Learn and Drill can't disagree. Before building, confirm levels with Sohyun. |
| 17 | Real-browser look at the Phase 94 Note Names drill after push | Quick -- after Sohyun pushes `7072051` | Especially Game (pixel) mode: the Press Start 2P font could not load in the sandbox test. Also try a `lock=1` student link on a phone. |
