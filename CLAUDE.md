...
| 16 | drills.html: Key Signatures, Notes & Rests and Rhythm Cards built (Phase 96) | Done -- review | Sohyun to try them in the preview/live site and flag anything off. Scale-card chapter (from the scale-card photo) not built -- blocked on verifying C#/Db, Eb, Ab fingerings (#15). |
| 17 | Real-browser look at the Phase 94-96 drills after push (esp. pixel font, tap-along timing on a phone) | Quick -- after Sohyun pushes `7072051` | Especially Game (pixel) mode: the Press Start 2P font could not load in the sandbox test. Also try a `lock=1` student link on a phone. |
