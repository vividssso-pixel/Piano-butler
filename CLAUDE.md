...
**Live-verified**: pending Sohyun's push.
...
| Random Pick + local Lists (Phase 71) | **Live-verified 2026-09-15** end to end (create list, add piece, My Lists panel) — confirmed working, no console errors. One real bug found in the same feature and fixed same session (see Sight-reading generator + Lists confirm() bug row below). | A `window.confirm()` bug is fixed in the working tree but **not yet pushed** — the live site still has the old behavior until Sohyun pushes. |
...
| # | Task | Priority | Notes |
|---|------|----------|-------|
| 1 | Sohyun — set up a reliable free keep-alive ping (UptimeRobot or similar) | High — Sohyun (needs an external account, Claude can't create one) | GitHub Actions' `render-keepalive.yml` cron (Phase 73) was confirmed unreliable for sub-hourly schedules (fired 2 times in hours, not every 10 min per its own schedule). A free UptimeRobot (or cron-job.org, etc.) monitor hitting `https://piano-butler-sightreading.onrender.com/api/health` every 5 minutes would keep the instance awake far more consistently — fewer cold starts for visitors, and Phase 80's excerpt pool stays full more reliably too. Claude cannot create third-party accounts on Sohyun's behalf; she signs up and adds the monitor herself (Claude can give exact steps if needed). |
| 2 | Live-verify Phase 80's excerpt pool once pushed | High | Sohyun needs to `git push` commit `eeea8ea` (+ CLAUDE.md log) first. Then confirm: (a) a Generate click well after boot/idle returns near-instantly (pool hit), (b) the response still renders correctly end-to-end (notation/PDF/audio), (c) nothing broke for a grade whose pool happens to be empty (live-compile fallback still works). |
