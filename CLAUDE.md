...
| # | Task | Priority | Notes |
|---|------|----------|-------|
| 1 | Live-verify Phase 77's header-cache + background-audio changes once pushed | High — needs Render redeploy | Confirm the header cache doesn't leak stale grade/realm text across different excerpts, and that the audio player reliably appears via polling a few seconds after the notation. |
