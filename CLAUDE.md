...
| # | Task | Priority | Notes |
|---|------|----------|-------|
| 1 | Live-verify Phase 84, 85 and 86 once pushed | High | Not yet pushed -- confirm on the live site: timeline.html's entry button reads "Piano exam" (not "AMEB Piano exam", `21214f4`); the Sight-Reading Generator's header "Piano Butler" logo now navigates to the homepage when clicked (`1e214e3`); and timeline.html's new free-text label + focus-area checklist flow works end to end for a real user (Phase 86, `21423a5` -- already live-tested in headless Chromium this session, but per this project's standing rule a locally-run test is not a substitute for a real spot-check on the deployed site). |
