...
| # | Task | Priority | Notes |
|---|------|----------|-------|
| 1 | Live-verify Phase 75's sight-reading speed changes once pushed | High — needs Render redeploy | Confirm the hands-view toggle still renders correctly on demand (`/api/sightreading-view`), and get a rough sense of whether generate feels faster for a typical (non-cold) request. |
| 2 | Live-verify Phase 76's loading page once pushed | High — needs Render redeploy | Confirm `sight-reading-loading.html` correctly detects the real app coming up via `/api/health` and redirects, on both a warm and (if possible to trigger) a cold instance. |
...
*Removed from this list 2026-09-15: Search Console indexing monitoring (was flat, now climbing —
...
`butler.html`'s scope (done, Phase 74 — stays private). Removed 2026-08-17:
