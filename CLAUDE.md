...
| # | Task | Priority | Notes |
|---|------|----------|-------|
| 1 | Live-verify Phase 82's `sight-reading.html` CTA revert and `timeline.html` generic mode once pushed | High | Both committed locally (`336edfa`, `55816fa`), not yet pushed — need a real click-through on the live site: sight-reading.html's Generate button should open the generator directly (no loading-page detour), and timeline.html's new mode-choice screen should route correctly to both the AMEB flow (unchanged) and the new generic performance flow. |
