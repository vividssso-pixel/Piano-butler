...
| 15 | Verify remaining DRAFT black-root scale fingerings (C#/Db, Eb, Ab) | Medium — Sohyun reviewing as she goes | Phase 90 (2026-09-19): Bb confirmed exact against the real AMEB scan, and F#/Gb was corrected after Sohyun caught it being wrong (3-consecutive-black-key run broke the generic formula). C#/Db, Eb, and Ab still use the same generic formula but haven't been individually checked -- they may have similar black-key-run exceptions. Sohyun's own workflow for this: she flags what's wrong as she reviews each key live in the app.
| 16 | drills.html: next concept set = Key Signatures (Tempo Race done first, Phase 95) (infographic + drill) on the Phase 94 engine | Next build -- agreed 2026-09-23 | Reuse `KeySigStaff`, `SHARP_ORDER`/`FLAT_ORDER` (already PDF-checked in Phase 90) as the shared data so Learn and Drill can't disagree. Before building, confirm levels with Sohyun. |
| 17 | Real-browser look at the Phase 94/95 drills (Note Names, Tempo Race) after push | Quick -- after Sohyun pushes `7072051` | Especially Game (pixel) mode: the Press Start 2P font could not load in the sandbox test. Also try a `lock=1` student link on a phone. |
